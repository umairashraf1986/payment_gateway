<?php
	...
	if(isset($_POST['search'])){
		...
		if($merchanr_list!='') 
			$where_clause .= "merchant_id = '".$merchanr_list."' AND ";
		else{
	...
	//////////////// /Paging Code Part-1 Start //////////////////////////////////////////////////////////
		...
		$qry_search_transaction = "SELECT 
								   ".$tblprefix."refund_request.*, ".$tblprefix."transaction_log.curr_code
								   FROM 
								   ".$tblprefix."refund_request
								   INNER JOIN
								    ".$tblprefix."transaction_log
									ON
									".$tblprefix."transaction_log.rp_id = ".$tblprefix."refund_request.rp_id
								   $where_clause 
								   ORDER BY 
								   id 
								   DESC 
...
<table width="100%" border="0" cellspacing="0" cellpadding="5" class="txt">
  ...
  <tr>
    ...
    <tr>
        <td align="left" colspan="2">
            <a href="export_to_csv_refund.php?refund_str=<?php echo base64_encode($where_clause) ?>" target="_parent">Export to CSV:</a></td>
        
        <td colspan="10" align="right">
        <form id="form_search" name="form_search" action="admin.php?act=refund_log" method="post">
  ...
  <tr id="heading"> 	
    ...
    <?php 
	if ($pageNum  > 0) {?>
    	<a class="pagination" href="<?php echo $_SERVER['PHP_SELF']; ?>?act=refund_log&src=<?php echo base64_encode($where_clause); ?>&pageNum=<?php echo max(0, $pageNum - 1)?>" > [Previous]</a>
    <?php 
	} ?>
  &nbsp;&nbsp;
  <?php 
	...
	if($pageNum>6)	
 	{	?>
  		<a class="paging" href="<?php echo $_SERVER['PHP_SELF']; ?>?act=refund_log&src=<?php echo base64_encode($where_clause); ?>&pageNum=<?php echo '0' ?>" > <?php echo "[First]"; ?> </a> &nbsp;
  <?php 
  	} 		
	for ($i=$startPage; $i< $count; $i=$i+1){
	 	if ($i!=$pageNum){
		?>
  			<a class="paging" href="<?php echo $_SERVER['PHP_SELF']; ?>?act=refund_log&src=<?php echo base64_encode($where_clause); ?>&pageNum=<?php echo $i; ?>"> <?php echo $i+1; ?> </a>
  <?php 
		}else{
			echo ("[". ($i + 1) ."]");
		}
	...
	if($pageNum+6<$totalPages)	{	?>
  		<a class="paging" href="<?php echo $_SERVER['PHP_SELF']; ?>?act=act=refund_log&src=<?php echo base64_encode($where_clause); ?>&pageNum=<?php echo $totalPages-1 ?> "> <?php echo "[Last]"; ?></a> &nbsp;
  <?php 
  	} ?>
  &nbsp;&nbsp;
  <?php 
  	if ($pageNum < $totalPages - 1) { ?>
  		<a class="paging" href="<?php echo $_SERVER['PHP_SELF']; ?>?act=refund_log&src=<?php echo base64_encode($where_clause); ?>&pageNum=<?php echo min($totalPages, $pageNum + 1)?>">[Next] </a>
  <?php 
  	} ?>
</div>
		</td>
