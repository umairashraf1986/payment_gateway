...
var imgdir = "http://192.168.1.17/game/images/cimg/"; // identify directory where captcha images are located
...
document.write("<p><input type=\"text\" id=\"" + jfldid + "\" name=\"" + jfldid + "\" class=\"form_input" + jfldcls + "\" size=\"" +  jfldsz + "\"><\/p>");
document.write("<div class='captcha_box'><img src=\"" + decodeURIComponent(imgdir) + imgid + ".jpg\" width=\"217\" height=\"41\" alt=\"\"><\/div>");
}
 function LTrim(text) {
...
//var flength = jQuery('#user_fullname').val().length;


/*var userfullname = jQuery('#user_fullname').val();
    userfullname = LTrim(userfullname);*/
...
/*if(userfullname== '')
	{
		//alert('Please entert full name!');
		jAlert('Please enter funllname.' , 'Full Name');
		jQuery('#user_fullname').focus();
		jQuery('#user_fullname').css('border-color', 'red');
		return false;
	...
	var illegalChars = /\W/;
	//alert(illegalChars.test(jQuery('#user_fullname').val));
	//var x =jQuery('#user_fullname').val();

	if(!fname.test( jQuery('#user_fullname').val())) {
		jAlert('Only Alphabetic character allowed' , 'Full Name');
		jQuery('#user_fullname').focus();
		jQuery('#user_fullname').css('border-color', 'red');
		return false;
	...
	/*if(!isNaN(jQuery('#user_fullname').val()))
	{
		jAlert('Full name can not be number.' , 'Full Name');
		jQuery('#user_fullname').focus();
		jQuery('#user_fullname').css('border-color', 'red');
		return false;
		//errorMsg+='Please Enter User Name<br>';
...
if(username== '')
	{
		//alert('Please entert full name!');
		jAlert('Please enter username.' , 'User Name');
		jQuery('#user_username').focus();
		jQuery('#user_username').css('border-color', 'red');
		
		return false;
		//errorMsg+='Please Enter User Name<br>';
	}
if(illegalChars.test( jQuery('#user_username').val())) {
		
		jAlert('Only Number , Letters underscore allowed' , 'User Name');
		jQuery('#user_username').focus();
		jQuery('#user_username').css('border-color', 'red');
		return false;
	...
	/*if(!isNaN(jQuery('#user_username').val()))
	{
		jAlert('User name can not be number.', 'User Name');
		jQuery('#user_username').focus();
		jQuery('#user_username').css('border-color', 'red');
		return false;
		//errorMsg+='Please Enter User Name<br>';
	 ...
	 if(useremail == '')
	  {
		jAlert('Please enter email address.', 'Email Address');
	   ...
	   if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(jQuery('#user_email').val()))
		{
			jAlert("Please enter valid email address.", 'Email Address');
			return false;
		}
	...
	{
		//alert('Please entert full name!');
		jAlert('Please enter password', 'User Password');
		jQuery('#user_password').focus();
		jQuery('#user_password').css('border-color', 'red');

		return false;
		//errorMsg+='Please Enter User Name<br>';
	}
	if( jQuery('#user_confpassword').val()== '')
	{
		//alert('Please entert full name!');
		jAlert('Please enter confirm password', 'User Password');
		jQuery('#user_confpassword').focus();
		jQuery('#user_confpassword').css('border-color', 'red');

		return false;
		//errorMsg+='Please Enter User Name<br>';
	}
	if( jQuery('#user_confpassword').val() != jQuery('#user_password').val())
		{
			jAlert('password is not matching' , 'Password Match');
			jQuery('#user_confpassword').focus();
			jQuery('#user_confpassword').css('border-color', 'red');
			return false;
			//errorMsg+='Please Enter User Name<br>';
	...
	if (uword==cword[anum-1]) {
		return true;
			...
			//$("#alert_button").click( function() {
							jAlert('ERROR: Enter the code as it is shown', 'Captha Dialog');
						//});
			return false;
