...
function htmlmail($to_email, $to_name, $from_email, $from_name, $subject, $message, $headers = NULL){
	...
	####################################################*/
	$qry = mysql_query("SELECT name,email FROM noz_admin Where type = 'admin'");
...
function gethotdealername($tblprefix, $id){
	if($id){
		global $db;
		$qry = "select * from ".$tblprefix."managehotel where hotel_id = '".$id."'";
		$rs = $db->Execute($qry);
		$isrs=$rs->RecordCount();
		if($isrs > 0){
			$qry_user = "select * from ".$tblprefix."users where id = '".$rs->fields['user_access_id']."'";
...
function getdicount($tblprefix, $id){
	if($id){
		global $db;
		$qry = "select * from ".$tblprefix."discount where id = '".$id."'";
		$rs = $db->Execute($qry);
		$isrs=$rs->RecordCount();
		if($isrs > 0){
...
function getcountry($tblprefix, $id){
	if($id){
		global $db;
		 $qry = "select id, CountryName from ".$tblprefix."country where id = '".$id."'";
		 $rs = $db->Execute($qry);
		$isrs=$rs->RecordCount();
		if($isrs > 0){
...
function gettravcountry_rec($tblprefix, $id,$colname){
	if($id){
		global $db;
		 $qry = "select ".$colname." from ".$tblprefix."trav_countries where country_id = '".$id."'";
		 $rs = $db->Execute($qry);
		 $isrs=$rs->RecordCount();
		if($isrs > 0){
...
function gettravcountryname($tblprefix, $id){
	if($id){
		global $db;
		 $qry = "select country_name from ".$tblprefix."trav_countries where country_id = '".$id."'";
		 $rs = $db->Execute($qry);
		 $isrs=$rs->RecordCount();
		if($isrs > 0){
...
function gettravcountryname_ar($tblprefix, $id){
	if($id){
		global $db;
		 $qry = "select country_name_ar from ".$tblprefix."trav_countries where country_id = '".$id."'";
		 $rs = $db->Execute($qry);
		 $isrs=$rs->RecordCount();
		if($isrs > 0){
...
function getdestname($tblprefix, $id){
	if($id){
		global $db;
		 $qry = "select destination from ".$tblprefix."car_destinations where dest_id = '".$id."'";
		 $rs = $db->Execute($qry);
		 $isrs=$rs->RecordCount();
		if($isrs > 0){
...
function getzonename($tblprefix, $id){
	if($id){
		global $db;
		 $qry = "select zone_name from ".$tblprefix."car_zones where zone_id = '".$id."'";
		 $rs = $db->Execute($qry);
		 $isrs=$rs->RecordCount();
		if($isrs > 0){
...
function getcountname($tblprefix, $id){
	if($id){
		global $db;
		 $qry = "select country_name from ".$tblprefix."car_destinations where  dest_id = '".$id."'";
		 $rs = $db->Execute($qry);
		 $isrs=$rs->RecordCount();
		if($isrs > 0){
			return $rs->fields['country_name'];
...
function commonTable($tblprefix, $tablename, $selectField, $wherefld, $whereval){
	global $db;
	$qry = "SELECT $selectField FROM ".$tblprefix.$tablename." WHERE $wherefld = '".$whereval."'";
	$rs = $db->Execute($qry);
...
function getemailvalidation($tblprefix, $mailid){
	...
	if($mailid){
		global $db;
		//email query
		$qry = "select * from ".$tblprefix."regcompanies where domain = '".$checkemail."'";
		$rs = $db->Execute($qry);
		$isrs=$rs->RecordCount();
		if($isrs > 0){
			return "True";
...
function gettypename($tblprefix, $typeid){
	if($typeid){
		global $db;
		$sql = "select * from ".$tblprefix."type where value = '".$typeid."'";
		$rs = $db->Execute($sql);
		$isrs = $rs->RecordCount();
		if($isrs > 0){
...
function getimage($tblprefix, $hotelid){
	if($hotelid){
		global $db;
		$sql = "select * from ".$tblprefix."hotelimages where hotel_id = '".$hotelid."'";
		$rs = $db->Execute($sql);
		if($rs){
...
function gethotelname($tblprefix, $hotelid){
	if($hotelid){
		global $db;
		$sql = "select * from ".$tblprefix."managehotel where hotel_id = '".$hotelid."'";
		$rs = $db->Execute($sql);
		if($rs){
...
function getstartdate($tblprefix, $hotelid){
	if($hotelid){
		global $db;
		$sql = "select * from ".$tblprefix."rates where hotel_id = '".$hotelid."'";
		$rs = $db->Execute($sql);
		if($rs){
...
function getdealername($tblprefix, $id){
	if($id){
		...
		$sql = "select * from ".$tblprefix."cardealers where cardealerid = '".$id."'";
		$rs = mysql_query($sql);
		//$rs = $db->Execute($sql);
		//$isrs = $rs->RecordCount();
...
function getbannername($tblprefix, $page){
	if($page){
		global $db;
		$qry = "select * from ".$tblprefix."banner_manage where applyto = '".$page."'";
		$rs = $db->Execute($qry);
		$isrs=$rs->RecordCount();
		if($isrs > 0){
...
function getroomname($tblprefix, $id){
	if($id){
		global $db;
		 $qry = "select * from ".$tblprefix."rooms where id = '".$id."'";
		 $rs = $db->Execute($qry);
		 $isrs=$rs->RecordCount();
		if($isrs > 0){
...
function getcompname($tblprefix, $email){
	if($email){
		...
		$qry = "select * from ".$tblprefix."regcompanies where domain = '".$imp_email."'";
		$rs = $db->Execute($qry);
		$isrs=$rs->RecordCount();
		if($isrs > 0){
...
function getroomtype($tblprefix, $type){
	if($type){
		global $db;
		$sql = "select * from ".$tblprefix."rooms where room_type = '".$type."'";
		$rs = $db->Execute($sql);
		if($rs){
...
function getroomtype_2($tblprefix, $id){
	if($id){
		global $db;
		$sql = "select * from ".$tblprefix."rooms where id = '".$id."'";
		$rs = $db->Execute($sql);
		if($rs){
			if($rs->fields['room_type'] == 'SWB'){
			return "Single";
...
$qr = "Select exchange_rate From noz_currencyrates Where id='$CodeID'";
$rec =  mysql_query($qr);
