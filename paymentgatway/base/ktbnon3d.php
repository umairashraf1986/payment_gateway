<?php
	...
	// LIVE: $url = "https://netpos.kuveytturk.com.tr/servlet/cc5ApiServer"

	if($payment_mode == 'Live') {
		$url = "https://netpos.kuveytturk.com.tr/servlet/cc5ApiServer";
		...
		$name = 'KUVEYTTURKAPI';       	   			// API Username  "eksimadmin"
		$password = 'KUVEYTTURK11';    	   		// API Pass = "Gr90dEt8"
		$clientid = '110000000';    		// VPOS MID clientid = "110000913"	
	} // END of ELSE
		
	
	if($api_mode == 1) {
		$lip=$customer_ip;  	// Merchant IP Address
		$G_merchant_website	= $merchant_web; // Merchant Website
	} else {	
	...
	$type = "Auth";   					 // Auth: Sale
	$ccno = $ccno; 				         // CC No
	$exp_date_month = $cc_exp_month;       // Exp Date Month  (format:MM)
	$exp_date_year = $cc_exp_year;         // Exp Date Year   (format:yyyy)
	$amount = number_format($amount,2);   	 // "." MUST be used as the decimal separator "10.05"
	$cv2 = $cc_cv2;                          // Card security code
